...
\begin{eqnarray}
\dot{M}  =& \mu R-\lambda_\text{M} M -\nu_\text{N} M & \\
\dot{S}  =& \lambda_\text{M} M - \lambda_\text{SI}SI-\lambda_\text{SP} SP &-\nu_\text{N} S + \mu S \\
\dot{I}  =& \lambda_\text{SI}SI+\lambda_\text{SP} SP - \lambda_\text{R} I &- \nu_\text{N} I \\
\dot{R}  =& \lambda_\text{R}I - \nu_\text{N} R +\mu_1S(t-\tau_1)I(t-\tau_1) &+\mu_1S(t-\tau_1)P(t-\tau_1) \\ 
\dot{P}  =&  \mu P -\nu_\text{P}P + \mu_2S(t-\tau_2)I(t-\tau_2) &+ \mu_2S(t-\tau_2)P(t-\tau_2)
\end{eqnarray}
following \citep{BAS16}. The time delays $(t-\tau_1)$ and $(t-\tau_2)$ are visualized in \ref{fig:infectionTimeScale}\footnote{This thing is super wrong according to "BVD Programm.doc"}. 
\begin{figure}[htbp]
\centering
